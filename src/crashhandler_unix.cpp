...
namespace g3 {
   ...
   namespace internal {
      ...
      void exitWithDefaultSignalHandler(const LEVELS& level, g3::SignalType fatal_signal_id) {
         ...
         exit(signal_number);
         // while a signal is being inside its handler, the same type of signals 
         // is blocked. Blocked signals are set to pending, but not queued. The 
         // term "pending" means that the operating system remembers that there 
         // is a signal waiting to be delivered at the next opportunity, and 
         // "not queued" means that it does this by setting a flag somewhere, 
         // but not by keeping an exact record of how many signals have arrived.
         // Thus, you may receive 2 or 3 (or 10) more SIGCHLD while in your signal
         // handler, but only see one
      }
