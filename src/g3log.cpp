...
namespace g3 {
   ...
   */
   ...
   std::function<void(void)> setFatalPreLoggingHook(std::function<void(void)>  pre_fatal_hook) {
       static std::mutex m;
       std::lock_guard<std::mutex> lock(m);
       auto previous_pre_fatal_hook = g_fatal_pre_logging_hook;
       g_fatal_pre_logging_hook = pre_fatal_hook;
       return previous_pre_fatal_hook;
   }
      ...
      * i.e. (dlopen + dlsym)  */
      void saveMessage(const char* entry, const char* file, int line, const char* function, const LEVELS& level,
         ...
         if (internal::wasFatal(level)) {
            ...
            auto fatalhook = setFatalPreLoggingHook(g_pre_fatal_hook_that_does_nothing);
