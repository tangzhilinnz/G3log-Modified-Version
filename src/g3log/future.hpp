...
namespace g3 {
   ...
   std::future<std::invoke_result_t<Func>> spawn_task(Func func, BgWorker* worker)
   {
      ...
      if (nullptr == worker) {
         ...
         // the associated std::future will be able to retrieve it just fine. 
         // It does not depend on std::promise still existing. If you failed 
         // to provide a value to std::promise before destroying it, trying
         // to get a result from the std::future will throw std::future_error,
         // but it is also well defined. 
         ...
         return future_result; // Move construct a std::future
      }
