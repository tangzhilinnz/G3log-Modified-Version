...
namespace g3 {
    ...
    *
    * Return the previous function-hook;
    * 
    * Example usage:
   ...
   std::function<void(void)> setFatalPreLoggingHook(std::function<void(void)>  pre_fatal_hook);
  ...
  namespace only_change_at_initialization {
   ...
   namespace internal {
      ...
      // custom fatal handler is done. This will make sure that the fatal message are pushed
...
// that (lexically) contains the point where they were created. This is true even if 
// that evaluation ends in throwing an exception. The value computations and side effects
// of destroying a temporary object are associated only with the full-expression, 
// not with any specific subexpression.
#define INTERNAL_LOG_MESSAGE(level) \
   LogCapture(__FILE__, __LINE__, static_cast<const char*>(__PRETTY_FUNCTION__), level)

#define INTERNAL_CONTRACT_MESSAGE(boolean_expression) \
   LogCapture(__FILE__, __LINE__, static_cast<const char*>(__PRETTY_FUNCTION__), g3::internal::CONTRACT, boolean_expression)


// LOG(level) is the API for the stream log
#define LOG(level) \
   if (!g3::logLevel(level)) {} else INTERNAL_LOG_MESSAGE(level).stream()
