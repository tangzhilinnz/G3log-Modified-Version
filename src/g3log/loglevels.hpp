...
// If you want to add any extra logging level then please add to your own source
// file the logging level you need
// 1. If the cmake option G3_DYNAMIC_LOGGING is enabled then you must use 
//    g3::only_change_at_initialization::addLogLevel(...).
//    to give g3log a record of your logging level and if it is an enabled 
//    or disbled logging level.
//
// 2. If the cmake dynamic logging option is turned OFF
//    then giving g3log a record of your logging level with 'addLogLevel(...) is 
//    NOT needed since no "disbled/enabled" check will happen - all logging levels
//    will be considered enabled.
// 3. See also the 
// [g3log/API.markdown](https://github.com/KjellKod/g3log/blob/master/API.markdown) 
// for for information.
//
...
//
// ... another example, when G3_DYNAMIC_LOGGING is disabled
...
const LEVELS 
   G3LOG_DEBUG {g3::kDebugValue, {/*"DEBUG"*/"DBG"}},
   INFO {g3::kInfoValue, {/*"INFO"*/"INF"}},
   WARNING {g3::kWarningValue, {/*"WARNING"*/"WRN"}},
   FATAL {g3::kFatalValue, {/*"FATAL"*/"FAT"}};
...
namespace g3 {
   namespace internal {
      const LEVELS 
         CONTRACT {g3::kInternalFatalValue, {"CONTRACT"}},
         FATAL_SIGNAL {g3::kInternalFatalValue + 1, {"FATAL_SIGNAL"}},
         FATAL_EXCEPTION {g3::kInternalFatalValue + 2, {"FATAL_EXCEPTION"}};

      /// helper function to tell the logger if a log message was fatal. If it is 
      /// it will force a shutdown after all log entries are saved to the sinks
