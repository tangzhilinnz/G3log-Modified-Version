...
TEST_F(RotateFileTest, CreateObject) {
   ...
   {
      FileRotateSink logrotate(_filename, _directory);
...
TEST_F(RotateFileTest, setMaxLogSizeAndRotate_ValidNewName) {
   ...
   auto allFiles = g3::internal::getArchiveLogFilesInDirectory(_directory, newFileName + ".log");
...
namespace {
   void RotateAndExpireOldLogs(std::string filename, std::string directory, bool changeLogFile = false) {
      FileRotateSink logrotate(filename, directory);
      ...
      for (size_t idx = 0; idx < 15; ++idx) {
         ...
         // force sleep 1s to trigger new time
...
TEST_F(RotateFileTest, rotateAndExpireOldLogs) {
   ...
   auto allFiles = g3::internal::getArchiveLogFilesInDirectory(_directory, app_name);
